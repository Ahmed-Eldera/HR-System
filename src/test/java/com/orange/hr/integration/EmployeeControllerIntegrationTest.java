...
public class EmployeeControllerIntegrationTest extends AbstractTest {
    ...
    private static final LocalDate HIRING_DATE = LocalDate.now();
    private static final int YOE = 0;
    ...
    public void addEmpolyeeSuccessfully_WithFullData_ExpectCreated() throws Exception {
        ...
        //assert
        result.andExpect(status().isCreated()).andExpect(jsonPath("$.employeeID").isNumber())
    ...
    public void addEmpolyee_WithDepartmentNotValid_ExpectNotFound() throws Exception {
        ...
        ResultActions result = mockMvc.perform(post("/employee")
                ...
                .content(objectMapper.writeValueAsString(employee)));
        //assert
        result.andExpect(status().isNotFound()).andExpect(jsonPath("$.msg")
                .value("Can't find the Selected Department"));
    }
    ...
    public void addEmpolyee_WithTeamNotValid_ExpectNotFound() throws Exception {
        ...
        //assert
        result.andExpect(status().isNotFound()).andExpect(jsonPath("$.msg")
                .value("Can't find the Selected Team"));
    }
    ...
    public void addEmpolyee_WithManagerNotValid_ExpectNotFound() throws Exception {
        ...
        //assert
        result.andExpect(status().isNotFound()).andExpect(jsonPath("$.msg")
                .value("Can't find the Selected Manager"));
    }
    ...
    public void addEmpolyee_WithBirthDateNotValid_ExpectNotFound() throws Exception {
        ...
        ResultActions result = mockMvc.perform(post("/employee")
                ...
                .content(objectMapper.writeValueAsString(employee)));
        //assert
        result.andExpect(status().isBadRequest()).andExpect(jsonPath("$.msg")
                .value("Birth date can't be in the future"));
    }
    ...
    public void addEmpolyee_WithExpertiseNotValid_ExpectNotFound() throws Exception {
        ...
        result.andExpect(status().isNotFound())
                .andExpect(jsonPath("$.msg")
                        .value("Can't find the Selected Expertise"));
    }
    ...
    public void addLeave_Given_YOE_LessThan10_ShouldReturnCreatedWithNoDeduction() throws Exception {
        ...
        double noOfadjustmentsBefore = adjustmentRepository.countByEmployeeAndDateGreaterThanEqualAndDateLessThan(
                employee,
                leaveDate,
                leaveDate.plusMonths(1));
        //act
        ...
        double noOfadjustmentsAfter = adjustmentRepository.countByEmployeeAndDateGreaterThanEqualAndDateLessThan(
                employee,
                leaveDate,
                leaveDate.plusMonths(1));
    ...
    @Test
    public void addLeave_GivenLeaveInPastMonth_ShouldReturnCreatedWithDeductionInThisMonth() throws Exception {
        prepareDB("/datasets/EmployeeController/AddLeave.xml");
        LocalDate fixedNow = LocalDate.of(2000, 3, 1);
        ...
        try (MockedStatic<LocalDate> date = Mockito.mockStatic(LocalDate.class, Mockito.CALLS_REAL_METHODS)) {
            ...
            Employee employee = employeeRepository.findById(EXISTING_EMPLOYEE_ID2).get();
            List<Leave> leaves = new ArrayList<>();
            LocalDate leaveDate = LocalDate.of(2000, 2, 01);
            //give the employee 30 leaves
            int maxAllowedLeaves = 30;
            for (int i = 1; i <= maxAllowedLeaves; i++) {
                leaves.add(new Leave(null, employee, leaveDate));
            ...
            double noOfadjustmentsAfter = adjustmentRepository.countByEmployeeAndDateGreaterThanEqualAndDateLessThan(
                    ...
                    adjustmentDate.plusMonths(1));
            //assert an adjustment (deduction) is made
            assertTrue(adjustmentRepository.existsByDate(LocalDate.now()));
            assertEquals(noOfadjustmentsBefore + 1, noOfadjustmentsAfter);
        }
    }

    @Test
    public void addLeave_GivenTheLeaveInTheNextYear_ShouldReturnBadRequest() throws Exception {
        prepareDB("/datasets/EmployeeController/AddLeave.xml");
        LocalDate fixedNow = LocalDate.of(2000, 3, 1);
        ...
        try (MockedStatic<LocalDate> date = Mockito.mockStatic(LocalDate.class, Mockito.CALLS_REAL_METHODS)) {
            date.when(LocalDate::now).thenReturn(fixedNow);
            //arrange
            LocalDate leaveDate = fixedNow.plusYears(1);
            LeaveRequestDTO leave = new LeaveRequestDTO(leaveDate);
            //act
            ResultActions result = mockMvc.perform(post("/employee/" + EXISTING_EMPLOYEE_ID2 + "/leave")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.setSerializationInclusion(JsonInclude.Include.NON_NULL).writeValueAsString(leave)));
            //assert
            result.andExpect(status().isBadRequest())
